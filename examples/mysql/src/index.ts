import { Condition, CRUDMySQL, Filter, MySQL, OffsetPagination, SortBy } from 'crud-node';
...
const example = async () => {
  ...
  const db = new MySQL(connection, settings);
  ...
  const officeController = new CRUDMySQL(db, officeSchema);
  ...
  await db.usingSession(async (session) => {
    const filterOfficesInNYC = Filter.toCriteria(
      Filter.and(
        Condition.like(`JSON_EXTRACT(address, '$."city"')`, '%New York%'),
        Condition.gre(OfficeProps.places, 1),
      ),
    );
